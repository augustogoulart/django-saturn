...
def test_urls_map_app_registered_models():
    ...
    assert (
        saturn_site.get_urls()[2].url_patterns[0].name == "saturn_themodel_changelist"
    )
...
@pytest.mark.parametrize("fields", ["name", "appLabel", "appUrl", "models"])
...
@pytest.mark.parametrize("fields", ["name", "objectName", "perms"])
